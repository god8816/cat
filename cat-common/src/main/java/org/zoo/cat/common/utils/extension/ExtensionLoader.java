...
package org.zoo.cat.common.utils.extension;
...
public final class ExtensionLoader<T> {
    ...
    private static <T> boolean withExtensionAnnotation(final Class<T> type) {
        return type.isAnnotationPresent(CatSPI.class);
    }
    ...
    public static <T> ExtensionLoader<T> getExtensionLoader(final Class<T> type) {
        if (type == null) {
            throw new CatException("type == null");
        }
        if (!type.isInterface()) {
            throw new CatException("Extension type(" + type + ") not interface!");
        }
        if (!withExtensionAnnotation(type)) {
            throw new CatException("type" + type.getName() + "not exist");
        }
    ...
    public T getActivateExtension(final String value) {
        ...
        return StreamSupport.stream(loader.spliterator(), false)
                .filter(e -> Objects.equals(e.getClass()
                        .getAnnotation(CatSPI.class).value(), value))
                .findFirst().orElseThrow(() -> new CatException("Please check your configuration"));
    }
