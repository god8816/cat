...
import org.zoo.cat.common.config.CatZookeeperConfig;
 ...
 */
@CatSPI("zookeeper")
public class ZookeeperCoordinatorRepository implements CatCoordinatorRepository {
    ...
    public int create(final CatTransaction catTransaction) {
        try {
            zooKeeper.create(buildRootPath(catTransaction.getTransId()),
                    RepositoryConvertUtils.convert(catTransaction, objectSerializer),
    ...
    public int update(final CatTransaction catTransaction) throws CatRuntimeException {
        try {
            ...
            zooKeeper.setData(buildRootPath(catTransaction.getTransId()),
                    RepositoryConvertUtils.convert(catTransaction, objectSerializer), -1);
            return ROWS;
        } catch (Exception e) {
            throw new CatRuntimeException(e);
        }
    ...
    public int updateParticipant(final CatTransaction catTransaction) {
        final String path = RepositoryPathUtils.buildZookeeperRootPath(rootPathPrefix, catTransaction.getTransId());
        try {
    ...
    public void init(final String modelName,final String appName, final CatConfig catConfig) {
        ...
        try {
            ...
            throw new CatRuntimeException(e.getMessage());
        }
    }

    private void connect(final CatZookeeperConfig config) {
        try {
