...
package org.zoo.cat.core.disruptor;
...
import org.zoo.cat.core.disruptor.event.DataEvent;
...
public class DisruptorProviderManage<T> {
    ...
    public void startup() {
        Disruptor<DataEvent<T>> disruptor = new Disruptor<>(new DisruptorEventFactory<>(),
                size,
                CatThreadFactory.create("disruptor_consumer_" + consumerFactory.fixName(), false),
                ProducerType.MULTI,
